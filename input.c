...
#define INIT_ORD_LINES 30  //колличество регистров занятых загрузчиком в оригинальной версии EDSAC
...
int main(int argc, char *argv[]) {
    ...
    int lastLine = 1; //Переменная, в которую сохранится номер послдедней строки и до которой будет читаться файл

    bool f = false;  // для создания массива стуктур,срабатывает один раз на 1 строчке и выделяется массив неоходимой длины
    struct Command *command;  //массив структур, в котором хранятся все

    while (lastLine != 0) {   // чтение из файла и заполнение массива структур
        ret = fscanf(in, " %d %c %d %c", &line, &instruction, &address, &length);

        if (ret == EOF) {
            printf("Incorrect address in first line");
            exit(12);  //такое может произойти только если номер изначально указанной последней строки больше чем номер последней строки в действительности
        }
        if (ret != 4) {
        ...
        if (!f) {
            ...
            f = !f; //срабатывает один раз на 1 строчке и выделяется массив неоходимой длины
    ...
    //самую последнюю запасную ячейку заполняю нулями, чтобы при обращений к ней(если вдруг) не было сюрпризов
    ...
    if (ret != EOF) {
        printf("Incorrect address in first line");
        exit(12);  //такое может произойти только если номер изначально указанной последней строки меньше чем номер последней строки в действительности
    }

    int stop_point = command[0].address -
                     32; //номер последней инструкции, сюда запишется номер строки где программа заканчивается
    ...
    //проверяю, корректен ли адрес перехода в условных опциях
    for (int k = 1; k < stop_point; ++k) {
